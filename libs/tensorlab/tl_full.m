function T = tl_full(T)
%FULL Convert formatted data set to an array.
%   T = fmt(T) converts the formatted data set T into a MATLAB array. If T
...
if ~isstruct(T)
    error('full:T','T must be a data set formatted by fmt.');
end

val = T.val;
size_tens = T.size;
if 8*prod(size_tens) > 8e9
    error('full:size','T is too large to be stored as an array.');
end
if ~isfield(T,'ind')
