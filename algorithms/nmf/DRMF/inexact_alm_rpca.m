...
[U diagS V] = svdex(D - E_hat + (1/mu)*Y, 1, struct('svd_solver','svds'));
while ~converged       
    iter = iter + 1;
    ...
    [U diagS V] = svdex(D - E_hat + (1/mu)*Y, sv, struct('init',U(:,1)));
    % if choosvd(n, sv) == 1
